...
## Hook v10: executable provenance

The v9 contract stated a causal rule — deleting every audible mark must leave the renderer with nothing to perform — but `resources/check_output.py` could only approximate it by guessing from attribute names (`data-role`, `data-t`, `data-voice`, …). A valid composition using `data-lane` was rejected as having "no visible semantic event marks", so it spent the one automatic repair turn producing bit-identical audio. About 1,300 words of fixed rules were also re-sent as user-message text on every turn.

Hook v10 makes the rule executable and moves fixed rules into the system prompt. The contract now reads: every sound-producing element of an original composition carries `data-audible="true"`.

- `resources/check_output.py` counts **visible `data-audible` marks** for `workflow=generation` (`svg.visibleEventMarks`) and reports their absence with the same wording as the silence test. The attribute-name heuristic remains only for the `reference` workflow. The file still never executes generated code.
- `resources/silence_check.py` runs the real test, for sound mode and `generation` only: it copies the version directory into a `tempfile.TemporaryDirectory()` (excluding the delivered audio, keeping local helpers), deletes every `data-audible` element from its copy of the SVG, and runs `render.py` on that empty score under bubblewrap — read-only bind of `/`, writable bind of the copy, `--dev /dev`, `--proc /proc`, `--tmpfs /tmp`, `--unshare-net`, `--die-with-parent`, `MPLCONFIGDIR` inside the copy, 180 s timeout. It passes only if the renderer exits zero, writes the audio file, and yields finite samples with peak ≤ 1e-4 (−80 dBFS). A renderer that hardcodes the absolute delivery path is rejected statically and never executed. If `/usr/bin/bwrap` is missing the test is skipped with a warning; generated code is never run unsandboxed. The real delivery directory is never written.
- Findings merge into `output-check.json` (`issues`, `warnings`) plus a `report.silence` detail block with marks/peak/seconds/ran. The existing single repair turn handles failures; its sound text now tells the model how to satisfy the silence test. When both scripts report a missing `data-audible` contract, `check_output.py` runs first and its wording is kept — the merge is de-duplicated, so the message appears once.
- `instructions.sound` is now `composer.md` + `workbench-context.md` + `output-context.md`, built once at startup. `beforeTurn` sends only per-turn facts: the reference-workflow pointer, `svg-composition-context.md` (generation) or `reference-context.md` plus helper and local excerpts (reference), and `research-context.md`. The Codex wrapper example lives in the new `resources/research-codex.md` and is injected only when the provider is not `claude`; `beforeTurn` takes `provider` for this. The hook audit version is 10; painting mode is unchanged. This supersedes the earlier per-turn description in *Working-context hook* above.
- Codex `thread/resume` and `claude.mjs` both re-send `developerInstructions` every turn (`systemPrompt.append`), so existing projects pick up the new system prompt without starting a new project. Each version stores SHA-256 hashes of the exact system prompt and per-turn context (`contextHook.systemPromptHash`, `contextHook.contextHash`) and writes `system-prompt.md` next to `context.md`, so later hook revisions can be compared by prompt identity. No benchmark runner is included.

What it does **not** prove: the silence test establishes provenance of sound — that delivered audio originates from the visible score — not musical quality, not that every audible detail is recoverable from geometry by a human, and not behaviour outside the sandbox. A 180 s timeout is inconclusive and reported as a warning, not a failure. Structural or static findings can be wrong for exotic but honest renderers; they cost at most the one bounded repair turn. No listening judgement is implied by a pass.

Restart Studio to load the hook, the new script and the new system prompt. Rollback and file inventory are in [the hook v10 checkpoint](checkpoints/2026-09-23-hook-v10-executable-provenance-uncommitted.md).
