...
export async function createSettings(dataDir, codex) {
  const file = path.join(dataDir, 'settings.json');
  let value = {provider:'default',openaiModel:'',openrouterModel:''};
  ...
  return {
    ...
    async selection() {
      if(value.provider==='openrouter') {
        ...
        return {provider:'openrouter',model:value.openrouterModel,threadConfig:{modelProvider:'studio_openrouter',model:value.openrouterModel,config:{'model_providers.studio_openrouter':{name:'OpenRouter',base_url:'https://openrouter.ai/api/v1',env_key:'OPENROUTER_API_KEY',wire_api:'responses',requires_openai_auth:false,supports_websockets:false}}}};
      }
